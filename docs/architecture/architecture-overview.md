...
Cortex does not replace the underlying agent runtime. Claude Code, Codex, and other MCP-capable hosts still run the agent itself. Cortex wraps the operational surface around that runtime so work is easier to inspect, coordinate, and audit.

## Related note

There is also an older high-level note in [architecture-reference.md](./architecture-reference.md).
