...
/**
 * Invoke `handler` when the user presses Enter or Space on a div that
 * behaves like a button. Prevents the default space-scroll behavior.
 */
const clickableKeyDown = (handler: () => void) => (e: KeyboardEvent<HTMLDivElement>) => {
...
export const AgentDetailCard = ({ name, model, platform, provider, requestCount, latency, cost, accentColor = 'purple', onClick, isSelected, hasSelection }: any) => {
  const cMap: any = {
  ...
  <GlassPanel
    ...
    {...(onClick
      ? {
          role: 'button',
          tabIndex: 0,
          onKeyDown: clickableKeyDown(onClick),
          'aria-pressed': isSelected ? true : undefined,
          'aria-label': `Select ${name}`,
        }
      : {})}
    className={`p-4 flex flex-col justify-between ${bColor} relative overflow-hidden transition-all duration-300 ${onClick ? 'cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400' : ''} ${isDim ? 'opacity-40 grayscale-[60%] border-transparent shadow-none' : activeStyles}`}
  >
...
export const ProjectCard = ({ project, onClick, onDelete, onComplete }: { project: ProjectData, onClick: () => void, onDelete?: (id: string) => void, onComplete?: (id: string) => void }) => {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [confirmComplete, setConfirmComplete] = useState(false);
  const isDone = DONE_STATUSES.has(project.status);
  return (
    <GlassPanel
       onClick={onClick}
       role="button"
       ...
       onKeyDown={clickableKeyDown(onClick)}
       className="p-6 group cursor-pointer hover:border-cyan-500/40 transition-all shadow-[0_4px_24px_-4px_rgba(0,0,0,0.5)] relative focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
    >
      <div className="absolute top-4 right-4 z-30 flex gap-2">
        {onComplete && project.progress >= 100 && (
          <button
             onClick={(e) => {
                e.stopPropagation();
                if (confirmComplete) {
             ...
             className={`h-6 px-2 rounded-full border transition-all shadow-md font-bold text-[8px] uppercase tracking-widest flex items-center justify-center ${confirmComplete ? 'bg-green-600 border-white text-white w-20' : 'bg-green-900/40 border-green-500/40 text-green-500 opacity-0 group-hover:opacity-100 hover:bg-green-600 hover:text-white w-16'}`}
          >
             {confirmComplete ? 'CONFIRM?' : 'CLOSE'}
          </button>
        )}
        {onDelete && (
          <button
             ...
             className={`h-6 px-2 rounded-full border transition-all shadow-md font-bold text-[8px] uppercase tracking-widest flex items-center justify-center ${confirmDelete ? 'bg-red-500 border-white text-white w-20' : 'bg-red-900/50 border-red-500/50 text-red-500 opacity-0 group-hover:opacity-100 hover:bg-red-500 hover:text-white w-6'}`}
          >
      ...
      <div className="flex justify-between items-start mb-4 relative z-20 pr-32">
        <div>
        ...
        <div className={`px-3 py-1 rounded text-[10px] tracking-widest font-mono shadow-inner uppercase border ${isDone ? 'bg-green-900/30 text-green-400 border-green-500/30' : 'bg-cyan-900/30 text-cyan-400 border-cyan-500/30'}`}>
      ...
      <div className="w-full bg-black/50 h-1.5 rounded-full overflow-hidden border border-white/10 relative z-20">
        <div 
          className={`h-full ${isDone ? 'bg-green-500 drop-shadow-[0_0_5px_#22c55e]' : 'bg-cyan-500 drop-shadow-[0_0_5px_#06b6d4]'} transition-all`}
