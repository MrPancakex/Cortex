...
import { BridgeMessageData, SystemLogData, TaskData } from '../../types/dashboard';

/**
 * Inline reply composer used by the Bridge feed. Replaces the old
 * `window.prompt()` call (Phase 9 Task 9.8) with a proper modal textarea
 * that supports Cmd/Ctrl+Enter to submit and Escape to close (Escape is
 * inherited from the parent Modal primitive).
 */
const ReplyComposer = ({ messageId, open, onClose, onSubmit }: { messageId: string; open: boolean; onClose: () => void; onSubmit: (id: string, body: string) => void }) => {
  ...
  useEffect(() => {
    if (open) {
      // Wait one frame so the Modal's focus trap runs first, then take focus.
  ...
  return (
    <Modal open={open} onClose={onClose} title="Reply to Bridge Message">
      <div className="space-y-4">
        <textarea
          ...
          placeholder="Draft your reply…  Cmd/Ctrl+Enter to send, Esc to cancel."
          className="w-full bg-black/50 border border-white/10 rounded px-4 py-2 text-sm text-white focus:border-purple-500/50 outline-none transition-colors font-mono"
        />
        <div className="flex justify-end space-x-3">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded text-[10px] font-mono uppercase tracking-widest text-gray-500 hover:text-white transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={submit}
            disabled={!body.trim()}
            className="px-6 py-2 rounded text-[10px] font-mono uppercase tracking-widest bg-purple-900/40 text-purple-400 border border-purple-500/30 hover:bg-purple-900/60 transition-colors shadow-[0_0_15px_rgba(168,85,247,0.2)] disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Send Reply
          </button>
        </div>
...
export const BridgeConnectivityPanel = ({ messages, agentColors = {}, hideTitle = false, compact = false, onNavigate, actions }: { messages: BridgeMessageData[], agentColors?: Record<string, string>, hideTitle?: boolean, compact?: boolean, onNavigate?: (route: string) => void, actions?: any }) => {
  ...
  <div className={`flex flex-col h-full bg-black/40 rounded-lg p-2 ${!compact ? 'border border-purple-500/10' : ''}`}>
      ...
      {!messages || messages.length === 0 ? (
        ...
        displayMsgs.map((m: BridgeMessageData) => (
          <div key={m.id} className="text-xs flex flex-col py-2 border-b border-white/5 last:border-0 hover:bg-white/5 px-2 rounded transition-colors relative group/msg">
            ...
            {!compact && (
              <button
                onClick={(e) => { e.stopPropagation(); setReplyTarget(m.id); }}
                className="absolute right-2 bottom-2 hidden group-hover/msg:block text-[8px] font-mono tracking-widest text-purple-400 uppercase bg-purple-900/40 px-2 py-0.5 rounded border border-purple-500/40 hover:bg-purple-900/80 transition-all font-bold shadow-[0_0_10px_rgba(168,85,247,0.3)]"
              >
                REPLY
              </button>
            )}
    ...
    <ReplyComposer
      ...
      onSubmit={(id, body) => actions?.replyMessage(id, body)}
    />
  </div>
  );
};

// `compact` is accepted for call-site compatibility (Overview renders a
// compact strip) even though the current ActivityTable implementation
// renders the same markup either way. Phase 10 may tighten this.
export const ActivityTable = ({ rows, compact: _compact = false }: { rows: SystemLogData[]; compact?: boolean }) => (
...
export const TaskHistoryTable = ({ tasks }: { tasks: TaskData[] }) => (
  <div className="w-full text-left text-sm text-gray-400 font-mono pt-1">
      ...
      <div key={task.id} className="flex items-center space-x-2 px-4 py-2 hover:bg-white/5 transition-colors border-b border-white/5 last:border-0 group">
        ...
        <div className="w-24 flex-none text-right">
           <span className={`text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded border border-current/20 bg-current/5 ${
             // Historical tables were written against the stale 'completed'
             // literal; TaskStatus only emits 'approved'. Keep the union
             // exhaustive so a future rename trips the typechecker.
             task.lifecycleStatus === 'approved' ? 'text-green-400' :
             task.lifecycleStatus === 'claimed' ? 'text-purple-400' :
             task.lifecycleStatus === 'review' ? 'text-blue-400' :
             task.lifecycleStatus === 'rejected' ? 'text-red-400' :
             task.lifecycleStatus === 'failed' ? 'text-red-500' :
             task.lifecycleStatus === 'in_progress' ? 'text-cyan-400' :
             task.lifecycleStatus === 'submitted' ? 'text-amber-400' :
             'text-gray-500'
           }`}>
